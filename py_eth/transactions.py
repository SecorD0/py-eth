...
class Tx(AutoRepr):
    ...
    def decode_input_data(self, client, contract: Contract) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
        ...
        try:
            self.function_identifier, self.input_data = Transactions.decode_input_data(
    ...
    def cancel(self, client, gas_price: Optional[types.GasPrice] = None,
        ...
        if self.params and 'nonce' in self.params:
            if not gas_price:
                gas_price = Transactions.gas_price(w3=client.w3).Wei
    ...
    def speed_up(self, client, gas_price: Optional[types.GasPrice] = None,
                 gas_limit: Optional[types.GasLimit] = None) -> bool:
        """
        ...
        if self.params and 'nonce' in self.params:
            if not gas_price:
                gas_price = int(Transactions.gas_price(w3=client.w3).Wei * 1.5)
...
class Transactions:
    ...
    def current_gas_price(w3: Web3) -> Wei:
        ...
        return Transactions.gas_price(w3=w3)

    @staticmethod
    def gas_price(w3: Web3) -> Wei:
        """
    ...
    @staticmethod
    def max_priority_fee(w3: Web3) -> Wei:
        """
        ...
        """
        return Wei(w3.eth.max_priority_fee)
    ...
    @staticmethod
    def decode_input_data(client, contract: types.Contract, input_data: Optional[str] = None,
    ...
    def auto_add_params(self, tx_params: TxParams) -> TxParams:
        ...
        if 'nonce' not in tx_params:
            tx_params['nonce'] = self.client.wallet.nonce()
        ...
        if 'gasPrice' not in tx_params and 'maxFeePerGas' not in tx_params:
            gas_price = self.gas_price(w3=self.client.w3).Wei
            ...
            tx_params['gasPrice'] = self.gas_price(w3=self.client.w3).Wei
    ...
    def send(self, token: types.Contract, recipient: types.Address, amount: types.Amount = 999_999_999_999_999,
        ...
        current_gas_price = self.gas_price(w3=self.client.w3)
        if not gas_price:
    ...
    def approve(self, token: types.Contract, spender: types.Address, amount: Optional[types.Amount] = None,
                gas_price: Optional[types.GasPrice] = None, gas_limit: Optional[types.GasLimit] = None,
